...
- **No DST (Daylight Saving Time) awareness in scheduling:** On March 8, 2026, Claude Desktop hit an infinite loop bug when computing next fire times during the DST spring-forward gap (2:00–3:00 AM). Atelier avoids that specific bug because it delegates timing to launchd's `StartCalendarInterval`, but tasks scheduled during a DST gap will silently not fire. Three things needed: (1) when `atelier-scheduler` is implemented, use `Calendar.nextDate(after:matching:matchingPolicy:)` with `.nextTimePreservingSmallerComponents` to correctly resolve DST gaps; (2) add a warning in `AutomationFormView` when users pick times in the 2:00–3:00 AM range; (3) add DST edge-case tests to the schedule test suite.
